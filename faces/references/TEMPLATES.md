...
Use `${face-alias}` in any message to reference a face's profile. The token is replaced with the face's display name in the message text, and the face's profile is injected as context for the model.
...
Note: `chat:chat` requires a face alias as the first argument, so bare model usage is only available via `chat:messages` and `chat:responses`.
